...
class KeywordExtractor:
    ...
    def extract_activity_keywords(self, text):
        """Extract activity-related noun and gerund keywords from text using spaCy"""
        doc = self.nlp(text)
        activities = []
        
        for token in doc:
            # Include nouns and verbs ending with 'ing' (gerunds)
            if (token.pos_ == 'NOUN' or 
                (token.pos_ == 'VERB' and token.text.lower().endswith('ing'))) and \
...
def demonstrate_keyword_extraction():
    user_prompt = "I want to go skiing, fishiing, and hiking."
