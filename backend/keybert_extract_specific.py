...
# ap = "I would like to go to bars with my friends. We want to go to a concert before getting drinks."
...
ap = "I want to eat at a Japanese restaurant"
...
preset_candidates2 = {
            ...
            "boba": ["boba", "bubble tea", "tea", "bubble tea shop", "boba tea shop", "bubble tea bar", "milk tea"],
            "cafes_bakeries": ["cafe", "coffee", "bakery", "pastry", "dessert", "brunch", "tea", "espresso", "croissant", "patisserie"],
            "bars_nightlife": ["bar", "nightlife", "club", "pub", "cocktail", "drink", "beer", "wine", "dance", "night", "party", "lounge", "alcohol", "liquor"],
            ...
            "entertainment_venues": ["entertainment", "theater", "cinema", "concert", "festival", "show", "performance", "music", "event", "venue", "rave"],
...
# Step 3: Check for adjective-noun combinations in extracted keywords
adjective_noun_combinations = []
for i in range(len(extracted_keywords_list)):
    for j in range(len(extracted_keywords_list)):
        if i != j:
            # Check if one keyword might be an adjective for another
            combined = f"{extracted_keywords_list[i]} {extracted_keywords_list[j]}"
            # Check if this combination exists in the original text
            if combined.lower() in ap.lower():
                adjective_noun_combinations.append({
                    'combined': combined.lower(),
                    'score': (extracted_keywords_dict[extracted_keywords_list[i]] + 
                             extracted_keywords_dict[extracted_keywords_list[j]]) / 2
                })

# Add specific categories for adjective-noun combinations
for combo in adjective_noun_combinations:
    specific_category = combo['combined'].replace(' ', '_')
    preset_candidates2[specific_category] = [combo['combined']]
    print(f"Added specific category: {specific_category}")

# Step 4: Calculate similarity between preset candidates and extracted keywords
results = []
...
for category, terms in preset_candidates2.items():
    ...
    for term_idx, term_embedding in enumerate(terms_embeddings):
        ...
        if max_sim > 0.65:  # Adjust this threshold as needed
